...
function renderMixer(){
  byId('mixer-ingredients').innerHTML=data.ingredients.map(item=>`<div class="ingredient-control"><b class="ingredient-icon">${item.icon}</b><span>${item.name}</span><div class="quantity-control"><button data-mix="-1" data-id="${item.id}">−</button><strong>${mixSelections[item.id]}</strong><button data-mix="1" data-id="${item.id}">+</button>${mixSelections[item.id]?`<button class="clear-ingredient" data-clear="${item.id}">×</button>`:''}</div></div>`).join('');
  byId('mixer-ingredients').querySelectorAll('.ingredient-control').forEach((card,index)=>{ const item=data.ingredients[index], count=mixSelections[item.id]; card.querySelector('.quantity-control strong').textContent=item.unit==='grams'?`${count*item.unitWeightG} g`:(item.id==='black-kismiss'?`${count} nos`:`${count} pc`); });
